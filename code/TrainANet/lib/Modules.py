...
class GPM(nn.Module):
    def __init__(self, dilation_series=[6, 12, 18], padding_series=[6, 12, 18], depth=64, in_channel=128):
        # def __init__(self, dilation_series=[2, 5, 7], padding_series=[2, 5, 7], depth=128):
        super(GPM, self).__init__()
        self.branch_main = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),
            BasicConv2d(in_channel, depth, kernel_size=1, stride=1)
        )
        self.branch0 = BasicConv2d(in_channel, depth, kernel_size=1, stride=1)
        self.branch1 = BasicConv2d(in_channel, depth, kernel_size=3, stride=1, padding=padding_series[0],
                                   dilation=dilation_series[0])
        self.branch2 = BasicConv2d(in_channel, depth, kernel_size=3, stride=1, padding=padding_series[1],
                                   dilation=dilation_series[1])
        self.branch3 = BasicConv2d(in_channel, depth, kernel_size=3, stride=1, padding=padding_series[2],

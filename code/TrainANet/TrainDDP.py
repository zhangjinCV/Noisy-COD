...
def train(rank, world_size, opt):
    ...
    train_dataset = PolypObjDataset(image_root=opt.train_root + 'image/', gt_root=opt.train_root + 'mask/', trainsize=opt.trainsize, istraining=True)
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset, num_replicas=world_size, rank=rank)
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=opt.batchsize, num_workers=6, sampler=train_sampler)
    ...
    for epoch in range(1, opt.epoch + 1):
        ...
        for i, (images, bbox_images, gts, edges) in enumerate(train_loader, start=1):
            ...
            if rank == 0 and (i % 20 == 0 or i == total_step or i == 1):
                ...
                writer.add_scalars('Loss_Statistics', {'Loss_init': loss_init.item(), 'Loss_final': loss_final.item(), 'Loss_edge': loss_edge.item(), 'Loss_total': loss.item()}, global_step=step)

        loss_all /= epoch_step
        if rank == 0:
            logging.info(f'[Train Info]: Epoch [{epoch:03d}/{opt.epoch:03d}], Loss_AVG: {loss_all:.4f}')
            writer.add_scalar('Loss-epoch', loss_all, global_step=epoch)
            if epoch > opt.epoch / 20 * 19:
...
def val(test_loader, model, epoch, save_path, writer, opt):
    ...
    with torch.no_grad():
        ...
        logging.info(f'[Val Info]:Epoch: {epoch}, MAE: {mae}, bestMAE: {opt.best_mae}, bestEpoch: {opt.best_epoch}.')
    torch.cuda.empty_cache()
...
def main():
    ...
    parser.add_argument('--top_epoch', type=int, default=10, help='epoch number')
    parser.add_argument('--top_lr', type=float, default=5e-4, help='learning rate')
    ...
    parser.add_argument('--ration', type=str, default='20', help='train ration')
    parser.add_argument('--train_root', type=str, default='../data/LabelNoiseTrainDataset/CAMO_COD_train_',
                        help='the training rgb images root')
    parser.add_argument('--val_root', type=str, default='../data/LabelNoiseTrainDataset/CAMO_COD_generate_',
    ...
    parser.add_argument('--save_path', type=str, default='../weight/ANet/', help='the path to save model and log')
